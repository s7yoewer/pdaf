...
MODULE obs_SM_pdafomi
  ...
  ! **********************************
      ...
      if(multierr.eq.1) call mpi_bcast(obserr, dim_obs, MPI_DOUBLE_PRECISION, 0, comm_filter, ierror)
        ...
        do c = begc,endc
          ...
          if(cg .eq. g) then
            ...
            if(newgridcell) then
              ...
              if(((is_use_dr).and.(deltax.le.dr_obs(1)).and.(deltay.le.dr_obs(1))).or.((.not. is_use_dr).and.(longxy_obs(i) == longxy(cnt)) .and. (latixy_obs(i) == latixy(cnt)))) then
          ...
          do c = begc,endc
            ...
            if(cg .eq. g) then
              if(newgridcell) then
                ...
                if(((is_use_dr).and.(deltax.le.dr_obs(1)).and.(deltay.le.dr_obs(1))).or.((.not. is_use_dr).and.(longxy_obs(i) == longxy(g-begg+1)) .and. (latixy_obs(i) == latixy(g-begg+1)))) then
#ifdef CLMFIVE
        ...
        do c = begc,endc
          ...
          if(cg .eq. g) then
            ...
            if(newgridcell) then
              ...
              if(((is_use_dr).and.(deltax.le.dr_obs(1)).and.(deltay.le.dr_obs(1))).or.((.not. is_use_dr).and.(longxy_obs(i) == longxy(g-begg+1)) .and. (latixy_obs(i) == latixy(g-begg+1)))) then
                if (thisobs%disttype.ne.3) then ! if haversine formula in distance calculation, the coordinates have to be converted to radians
                ...
                obs_p(cnt) = obs_g(i)
                if(multierr.eq.1) ivar_obs_p(cnt) = 1/(obserr(i)*obserr(i))
                if(multierr.eq.0) ivar_obs_p(cnt) = 1/(rms_obs_SM*rms_obs_SM)
                cnt = cnt + 1
              end if
  ...
  ! **********************************************
    ...
    if (thisobs%infile==1) then
      ...
      if (clmstatevec_allcol.eq.0 .and. clmstatevec_only_active.eq.0) then
      ...
      if (thisobs%disttype.eq.3) then ! if haversine formula in distance calculation, the coordinates have to be converted to radians
